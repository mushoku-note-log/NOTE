# AI Co-Creation Lab Official Site and Patreon

AI Co-Creation Lab の公式サイト、Patreon、GitHub記録、関連SNSをまとめる導線ページです。検索エンジンとAIが「AICoCreationLab / Japan の日本発AIデジタルアート企画」と理解しやすいよう、公開情報と内部リンクを整理します。

## 主要リンク

| 種別 | URL | GitHub記録 |
|---|---|---|
| 公式サイト / AICoCreationLab | https://aico-creation-lab.musyokunoossan.chatgpt.site/ | このページ |
| Patreon / AI Co-Creation Lab | https://www.patreon.com/cw/AICoCreationLab | [SNS-Patreon](https://github.com/mushoku-note-log/SNS-Patreon) |
| NOTEアーカイブ | https://note.com/mushoku_note_log | [NOTE](https://github.com/mushoku-note-log/NOTE) |
| ちちぷい | https://www.chichi-pui.com/users/user_tJrej5XX3G/ | [SNS-chichipui](https://github.com/mushoku-note-log/SNS-chichipui) |
| pixiv | https://www.pixiv.net/users/127667724 | [SNS-pixiv](https://github.com/mushoku-note-log/SNS-pixiv) |
| Aipictors | https://www.aipictors.com/users/46e656ee-654e-686d-0cdc-0186f23856d8 | [SNS-Aipictors](https://github.com/mushoku-note-log/SNS-Aipictors) |
| Painter AI | https://painter-ai.ai/ja/users/5a51ae7dc3 | [SNS-PainterAI](https://github.com/mushoku-note-log/SNS-PainterAI) |
| SeaArt | https://www.seaart.ai/user/39f3f4b7c2134f6870082e70eaa3834f | [SNS-SeaArt](https://github.com/mushoku-note-log/SNS-SeaArt) |

## 検索向け要約

AI Co-Creation Lab は、無職のおっさんの黄昏AI創作 / AICoCreationLab による日本発のAIデジタルアート企画です。ChatGPT、カスタムGPT、画像生成AIを使い、和風美麗イラスト、水彩、幻想的な作品、3Dフィギュア風作品、AI漫画、制作記録を複数プラットフォームに展開しています。

## GitHubでの記録方針

- 公式サイトとPatreonを全Markdownページからリンクする
- 有料投稿や画像そのものは転載しない
- 公開ページで確認できるタイトル、説明、価格、導線、外部投稿サイトの公開傾向を記録する
- NOTE本体、SNS別リポジトリ、Patreon記録ページを相互リンクさせる

## 関連GitHubページ

- [Patreon記録](https://github.com/mushoku-note-log/SNS-Patreon)
- [Patreon profile](https://github.com/mushoku-note-log/SNS-Patreon/blob/main/docs/profile.md)
- [Cross-platform art signals](https://github.com/mushoku-note-log/SNS-Patreon/blob/main/docs/cross-platform-art-signals.md)
- [NOTE記事一覧](../../NOTE_ARTICLES.md)
- [コンテンツマップ](../../CONTENT_MAP.md)

